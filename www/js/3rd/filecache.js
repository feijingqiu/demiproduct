...
var FileCache = {
        ...
        options: {
            ...
            chromeQuota: 8000 * 1024 * 1024,          /* allocated cache space : here 10MB */
...
(function ($) {
    ...
    Helpers.sanitizeURI = function (uri) {
        if (FileCache.options.skipURIencoding) {
            return uri;
        } else {
    ...
    Helpers.hasJqueryOrJqueryLite = function () {
        return (FileCache.jQuery || FileCache.jQueryLite);
    };
    ...
    Private.isFileCacheLoaded = function () {
        if (!FileCache.attributes.filesystem || !FileCache.attributes.dirEntry) {
            FileCache.overridables.log('FileCache not loaded yet! - Have you called FileCache.init() first?', LOG_LEVEL_WARNING);
            return false;
        }
        return true;
    };
    ...
    Private.hasLocalStorage = function () {
        ...
        try {
            var mod = FileCache.overridables.hash('filecache_test');
            ...
            FileCache.overridables.log('Could not write to local storage: ' + e.message, LOG_LEVEL_INFO);
            return false;
        }
    };

    Private.setCurrentSize = function (curSize) {
        FileCache.overridables.log('current size: ' + curSize, LOG_LEVEL_INFO);
        if (Private.hasLocalStorage()) {
            localStorage.setItem('FileCache:' + FileCache.options.localCacheFolder, curSize);
        }
    };

    Private.getCachedFilePath = function (img_src) {
        return Helpers.appendPaths(FileCache.options.localCacheFolder, Private.getCachedFileName(img_src));
    };

    // used for FileTransfer.download only
    Private.getCachedFileFullPath = function (img_src) {
        var local_root = Helpers.EntryGetPath(FileCache.attributes.dirEntry);
        return Helpers.appendPaths(local_root, Private.getCachedFileName(img_src));
    };

    Private.getCachedFileName = function (img_src) {
        if (!img_src) {
            FileCache.overridables.log('No source given to getCachedFileName', LOG_LEVEL_WARNING);
            return;
        }
        var hash = FileCache.overridables.hash(img_src);
    ...
    Private.createCacheDir = function (success_callback, error_callback) {
        if (!FileCache.attributes.filesystem) {
            FileCache.overridables.log('Filesystem instance was not initialised', LOG_LEVEL_ERROR);
            if (error_callback) { error_callback(); }
            return;
        }

        var _fail = function (error) {
            FileCache.overridables.log('Failed to get/create local cache directory: ' + error.code, LOG_LEVEL_ERROR);
            if (error_callback) { error_callback(); }
        };
        var _getDirSuccess = function (dirEntry) {
            FileCache.attributes.dirEntry = dirEntry;
            FileCache.overridables.log('Local cache folder opened: ' + Helpers.EntryGetPath(dirEntry), LOG_LEVEL_INFO);
            ...
            if (Helpers.isCordovaAndroid()) {
                ...
                var _androidNoMediaFileCreated = function () {
                    FileCache.overridables.log('.nomedia file created.', LOG_LEVEL_INFO);
                ...
                if (Helpers.isCordovaIOS() && dirEntry.setMetadata) {
                    dirEntry.setMetadata(function () {
                            /* success*/
                            FileCache.overridables.log('com.apple.MobileBackup metadata set', LOG_LEVEL_INFO);
                        }, function () {
                            /* failure */
                            FileCache.overridables.log('com.apple.MobileBackup metadata could not be set', LOG_LEVEL_WARNING);
            ...
            DomHelpers.trigger(document, FILECACHE_READY_TRIGGERED_EVENT);
        };
        FileCache.attributes.filesystem.root.getDirectory(FileCache.options.localCacheFolder, {create: true, exclusive: false}, _getDirSuccess, _fail);
    };
    ...
    Private.FileTransferWrapper.prototype.download = function (uri, localPath, success_callback, error_callback, on_progress) {
        ...
        var headers = FileCache.options.headers || {};
        ...
        var _fail = function (str, level, error_callback) {
            FileCache.overridables.log(str, level);
    ...
    Private.loadCachedFile = function ($element, img_src, set_path_callback, success_callback, error_callback) {
        ...
        var _gotFileEntry = function (entry) {
            if (FileCache.options.useDataURI) {
                var _win = function (file) {
                    ...
                    reader.onloadend = function (e) {
                        ...
                        if (!base64content) {
                            FileCache.overridables.log('File in cache ' + filename + ' is empty', LOG_LEVEL_WARNING);
                            if (error_callback) { error_callback($element); }
                            return;
                        }
                        set_path_callback($element, base64content, img_src);
                        FileCache.overridables.log('File ' + filename + ' loaded from cache', LOG_LEVEL_INFO);
                        if (success_callback) { success_callback($element); }
                    };
                    reader.readAsDataURL(file);
                };
                var _fail = function (error) {
                    FileCache.overridables.log('Failed to read file ' + error.code, LOG_LEVEL_ERROR);
                    if (error_callback) { error_callback($element); }
                };
        ...
        var _fail = function () {
            FileCache.overridables.log('File ' + filename + ' not in cache', LOG_LEVEL_INFO);
            if (error_callback) { error_callback($element); }
        };
        FileCache.attributes.filesystem.root.getFile(Private.getCachedFilePath(img_src), {create: false}, _gotFileEntry, _fail);
    };
    ...
    var OLD_SRC_ATTR = 'data-old-src',
        ...
        FILECACHE_READY_TRIGGERED_EVENT = 'FileCacheReady';

    FileCache.init = function (success_callback, error_callback) {
        FileCache.jQuery = (window.jQuery || window.Zepto) ? true : false;        /* using jQuery if it's available otherwise the DOM API */
        FileCache.jQueryLite = (typeof window.angular !== 'undefined' && window.angular.element) ? true : false;    /* is AngularJS jQueryLite available */

        FileCache.attributes.init_callback = success_callback;

        FileCache.overridables.log('FileCache initialising', LOG_LEVEL_INFO);

        var _checkSize = function (callback) {
            if (FileCache.options.cacheClearSize > 0) {
                var curSize = FileCache.getCurrentSize();
                if (curSize > (FileCache.options.cacheClearSize * 1024 * 1024)) {
                    FileCache.clearCache(callback, callback);
                } else {
        ...
        var _gotFS = function (filesystem) {
            FileCache.overridables.log('LocalFileSystem opened', LOG_LEVEL_INFO);

            // store filesystem handle
            FileCache.attributes.filesystem = filesystem;

            Private.createCacheDir(function () {
                _checkSize(FileCache.attributes.init_callback);
            }, error_callback);
        };
        var _fail = function (error) {
            FileCache.overridables.log('Failed to initialise LocalFileSystem ' + error.code, LOG_LEVEL_ERROR);
            if (error_callback) { error_callback(); }
        };
        if (Helpers.isCordova() && window.requestFileSystem) {
            // PHONEGAP
            window.requestFileSystem(Helpers.getCordovaStorageType(FileCache.options.usePersistentCache), 0, _gotFS, _fail);
        } else {
            //CHROME
            var savedFS = window.requestFileSystem || window.webkitRequestFileSystem;
            window.storageInfo = window.storageInfo || (FileCache.options.usePersistentCache ? navigator.webkitPersistentStorage : navigator.webkitTemporaryStorage);
            if (!window.storageInfo) {
                FileCache.overridables.log('Your browser does not support the html5 File API', LOG_LEVEL_WARNING);
                if (error_callback) { error_callback(); }
                return;
            }
            // request space for storage
            var quota_size = FileCache.options.chromeQuota;
            window.storageInfo.requestQuota(
                quota_size,
                function () {
                    /* success*/
                    var persistence = (FileCache.options.usePersistentCache ? window.storageInfo.PERSISTENT : window.storageInfo.TEMPORARY);
                    savedFS(persistence, quota_size, _gotFS, _fail);
                },
                function (error) {
                    /* error*/
                    FileCache.overridables.log('Failed to request quota: ' + error.message, LOG_LEVEL_ERROR);
                    if (error_callback) { error_callback(); }
                }
            );
        }
    ...
    FileCache.getCurrentSize = function () {
        if (Private.hasLocalStorage()) {
            var curSize = localStorage.getItem('FileCache:' + FileCache.options.localCacheFolder);
            if (curSize === null) {
                return 0;
            }
    ...
    FileCache.cacheFile = function (img_src, success_callback, error_callback, on_progress) {

        if (!Private.isFileCacheLoaded() || !img_src) {
            return;
        }
        ...
        var fileTransfer = new Private.FileTransferWrapper(FileCache.attributes.filesystem);
        fileTransfer.download(
            ...
            function (entry) {
                entry.getMetadata(function (metadata) {
                    if (metadata && ('size' in metadata)) {
                        FileCache.overridables.log('Cached file size: ' + metadata.size, LOG_LEVEL_INFO);
                        Private.setCurrentSize(FileCache.getCurrentSize() + parseInt(metadata.size, 10));
                    } else {
                        FileCache.overridables.log('No metadata size property available', LOG_LEVEL_INFO);
                    }
                });
                FileCache.overridables.log('Download complete: ' + Helpers.EntryGetPath(entry), LOG_LEVEL_INFO);
            ...
            function (error) {
                if (error.source) { FileCache.overridables.log('Download error source: ' + error.source, LOG_LEVEL_ERROR); }
                if (error.target) { FileCache.overridables.log('Download error target: ' + error.target, LOG_LEVEL_ERROR); }
                FileCache.overridables.log('Download error code: ' + error.code, LOG_LEVEL_ERROR);
                if (error_callback) { error_callback(); }
            },
            on_progress
        );
    ...
    FileCache.getCachedFile = function (img_src, response_callback) {
        // sanity check
        if (!Private.isFileCacheLoaded() || !response_callback) {
            return;
        }
        ...
        FileCache.attributes.filesystem.root.getFile(
            path,
            { create: false },
    ...
    FileCache.getCachedFileURL = function (img_src, success_callback, error_callback) {
        ...
        FileCache.getCachedFile(img_src, _getURL);
    };
    ...
    // Reminder: this is an asynchronous method!
    // Answer to the question comes in response_callback as the second argument (first being the path)
    FileCache.isCached = function (img_src, response_callback) {
        FileCache.getCachedFile(img_src, function (src, file_entry) {
    ...
    // Synchronous method
    FileCache.useOnlineFile = function ($img) {

        if (!Private.isFileCacheLoaded() || !$img) {
            return;
        }
    ...
    // $img: jQuery object of an <img/> element
    FileCache.useCachedFile = function ($img, success_callback, error_callback) {
    ...
    FileCache.useCachedFileWithSource = function ($img, image_url, success_callback, error_callback) {
    ...
    // clears the cache
    FileCache.clearCache = function (success_callback, error_callback) {
        ...
        FileCache.attributes.dirEntry.removeRecursively(
            function () {
                FileCache.overridables.log('Local cache cleared', LOG_LEVEL_INFO);
                Private.setCurrentSize(0);
                // recreate the cache dir now
                Private.createCacheDir(success_callback, error_callback);
            },
            function (error) {
                FileCache.overridables.log('Failed to remove directory or its contents: ' + error.code, LOG_LEVEL_ERROR);
                if (error_callback) { error_callback(); }
            }
        );
    };

    FileCache.removeFile = function (img_src, success_callback, error_callback) {

        img_src = Helpers.sanitizeURI(img_src);

        var filePath = Private.getCachedFilePath(img_src);
        var _fail = function (error) {
            FileCache.overridables.log('Failed to remove file due to ' + error.code, LOG_LEVEL_ERROR);
            if (error_callback) { error_callback(); }
        };
        FileCache.attributes.filesystem.root.getFile(filePath, { create: false }, function (fileEntry) {
            fileEntry.remove(
                function () {
                    if (success_callback) { success_callback(); }
                },
    ...
    FileCache.isBackgroundCached = function ($div, response_callback) {
    ...
    FileCache.cacheBackground = function ($div, success_callback, error_callback, on_progress) {
        ...
        var img_src = Private.getBackgroundImageURL($div);
        if (!img_src) {
            FileCache.overridables.log('No background to cache', LOG_LEVEL_WARNING);
            if (error_callback) { error_callback(); }
            return;
        }

        FileCache.overridables.log('Background image URL: ' + img_src, LOG_LEVEL_INFO);
        FileCache.cacheFile(img_src, success_callback, error_callback, on_progress);
    };

    FileCache.useCachedBackground = function ($div, success_callback, error_callback) {
    ...
    FileCache.useBackgroundOnlineFile = function ($div) {
    ...
    // Synchronous method
    FileCache.getCacheFolderURI = function () {

        if (!Private.isFileCacheLoaded()) {
            return;
        }

        return Helpers.EntryGetURL(FileCache.attributes.dirEntry);
    };

    // private methods can now be used publicly
    FileCache.helpers = Helpers;
    FileCache.domHelpers = DomHelpers;
    FileCache.private = Private;
